app.service("constructionService", function ($http) {
  ...
  this.getAllConstructions = function (
  ...
  //
  this.getAllAdministrativeUnit = function () {
    var response = $http({
      method: "get",
  ...
  this.getCommunes = function () {
    return this.getAllAdministrativeUnit().then(function (response) {
      var data = response.data.ListData;
      // Filter communes (Level == "Commune")
      var communes = data.filter(function (item) {
        return item.Level === "Commune";
      });
      return communes;
    });
  };
