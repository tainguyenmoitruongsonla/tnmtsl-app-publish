var App = function () {
    ...
    // Sự kiện khi load page
    var onLoadPage = function () {
    	
		$('.top-nav-toggle').click(function(){
				$(this).parent().addClass('open');
				var subObj = $(this).parent().find('.sub-top-nav');
				var rootObj = $(this).parent().parent();
				if(subObj.css('display') == 'none')
					subObj.show();
				else 
					subObj.hide();
				$(rootObj).find('.top-nav').each(function(){
					if(!$(this).hasClass('open'))
					{
						$(this).find('.sub-top-nav').hide();
					}
				});
			});
			
			var mouseInside = false;
			$('.sub-top-nav').hover(function(){ 
				mouseInside=true; 
			}, function(){ 
				mouseInside=false; 
			});
			...
			$("body").mouseup(function(){ 
				if(! mouseInside) $('.sub-top-nav').hide();		
			});
			
			$("body").mouseup(function(){ 
				if(! mouseTop ) $('#TopLayout').slideUp('fast');		
			});
			...
			if(winW >= 980){
				var headerH = $('#NavbarInner').height();
				var footerH = $('#Footer').height();
				var actionBarH = $('#TopActionBar').height();
				var mainH = Number(winH) - Number(headerH) - Number(footerH);
				var dashboardH = Number(mainH) - Number(actionBarH) - 21;
				$('#MainContainer').attr('style','height:' + mainH + 'px !important');
				$('#DashBoard').attr('style','height:' + dashboardH + 'px !important; overflow:auto;');
			}
			else
			{
				$('#MainContainer').attr('style','height:auto');
				$('#DashBoard').attr('style','height:auto');
				$('#SidebarSearch').parent().css('display','none');
			}
			$(window).resize(function () {
				...
				headerH = $('#NavbarInner').height();
				footerH = $('#Footer').height();
				actionBarH = $('#TopActionBar').height();
				mainH = Number(winH) - Number(headerH) - Number(footerH);
				dashboardH = Number(mainH) - Number(actionBarH) - 21;
				headerH = $('#NavbarInner').height();
				footerH = $('#Footer').height();
				actionBarH = $('#TopActionBar').height();
				mainH = Number(winH) - Number(headerH) - Number(footerH);
				...
				{					
					$('#MainContainer').attr('style','height:' + mainH + 'px !important');
					$('#DashBoard').attr('style','height:' + dashboardH + 'px !important; overflow:auto;');
					$('#SidebarSearch').parent().removeAttr('style');
...
*/
    ...
    var handleMainMenu = function () {
        ...
        jQuery('.page-sidebar .has-sub > a').click(function () {// 1
            ...
            setTimeout(CheckDivScrollBar,200);
        });
		// Ham thuc hien xu ly cac hieu ung scrollbar
        function CheckDivScrollBar()
	    {
	    	var api;
	    	if(checkRemove)
			{
				var leftMenu = $('#LeftSidebar').find('ul').first();
				api=$(leftMenu).data('jsp');
				var leftSideH = $('#MainContainer').height()-41;
				var leftMenuH = $(leftMenu).find('.jspPane').height();
				if((leftSideH>=leftMenuH)&&(leftMenuH!=null))
				{
					api.destroy();
					$(leftMenu).removeAttr('style');
				}
				else if(leftSideH<leftMenuH)
				{
					api=$(leftMenu).data('jsp');
					$(leftMenu).height(leftSideH);
					api.reinitialise();
				}
				checkRemove=false;
			}
			if(checkAdd)
			{
				...
				$(leftMenu).removeAttr('style');
				var leftSideH = $('#MainContainer').height()-41;
				var leftMenuH = $(leftMenu).height();
				var leftPaneH = $(leftMenu).find('.jspPane').height();
				if((leftSideH<=leftMenuH)&&(leftPaneH==null))
				{
					$(leftMenu).height(leftSideH);
					$(leftMenu).jScrollPane({});
				}
				else if(leftSideH<=leftPaneH)
				{
					api=$(leftMenu).data('jsp');
					$(leftMenu).height(leftSideH);
					api.reinitialise();
    ...
    var handleSidebarToggler = function () {
        ...
        if ($.cookie('sidebar-closed') == 1) {
            container.addClass("sidebar-closed");
            $('#ToggleSidebar i').removeClass('m-icon-swapleft');
            $('#ToggleSidebar i').addClass('m-icon-swapright');
            $('#ToggleSidebar').removeClass('redx');
			$('#ToggleSidebar').addClass('green');
			if($('#SidebarSearchButton').css('display') !== 'none')
				$('#SidebarSearchButton').css('display','none');
        }
        ...
        $('#ToggleSidebar').click(function () {
			...
			if (container.hasClass("sidebar-closed") === true) {
                ...
                $('#ToggleSidebar i').removeClass('m-icon-swapright');
                $('#ToggleSidebar').removeClass('green');
                $('#ToggleSidebar i').addClass('m-icon-swapleft');
                $('#ToggleSidebar').addClass('red');
                if($('#SidebarSearchButton').css('display') == 'none')
					$('#SidebarSearchButton').css('display','inline-block');		
                $.cookie('sidebar-closed', null);                
            } else {
                container.addClass("sidebar-closed");
                $('#ToggleSidebar i').removeClass('m-icon-swapleft');
			...
			var sidebarW = Number($('#LeftSidebar').width());
			$('#MainContainer').css('width',sidebarW + 'px !important');
			$('#DashBoard').css('width',sidebarW + 'px !important');
			$(window).trigger('resize');
        });
    }
	
	function ScrollBarLeftMenu()
	{
		if($.cookie('sidebar-closed') == null)
		{
			...
			{
				var api=$(leftMenu).data('jsp');
				$(leftMenu).height(leftSideH);
				api.reinitialise();
			}
			else if((leftSideH>=leftPaneH)&&(leftPaneH!=null))
			{
				var api=$(leftMenu).data('jsp');
				api.destroy();
				$(leftMenu).removeAttr('style');
		...
		{
			var leftMenu = $('#LeftSidebar').find('ul').first();
			if($(leftMenu).find('.jspTrack').is(':not(:hidden)'))
			{
				var api=$(leftMenu).data('jsp');
				api.destroy();
            	$(leftMenu).removeAttr('style');
	...
	// xu ly scrollbar khi thay doi kich thuoc windows
	$(window).resize(function () {
		ScrollBarLeftMenu();
	});
