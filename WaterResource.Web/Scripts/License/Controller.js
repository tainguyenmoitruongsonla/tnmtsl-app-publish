app.controller(
  ...
  function (
    ...
    $scope.DataYear = [];
    $scope.CountLicenses = defaultCountLicenses();
    ...
    // Format old license sign date: return blank for null/invalid or Date year <= 1
    $scope.formatOldLicenseDate = function (item) {
      if (!item || !item.OldLicense || !item.OldLicense.SignDate) return "";
      try {
        var d = new Date(item.OldLicense.SignDate);
    ...
    $scope.filterConsByType = function (Id, Name) {
      ...
      ConstructionId = 0;
      $scope.ConstructionFilter = "";
      SetInputVal("ConstructionFilter", 0);
    ...
    // Filter by commune (administrative unit)
    $scope.filterLicenseByCommune = function (Id, Name) {
      CommuneId = Id;
      CommuneCode = "";
    ...
    $scope.filterLicenseByCons = function (item, inputId) {
      if (item != 0) {
        ...
        $scope.ConstructionFilter = item.ConstructionName;
      } else {
        ConstructionId = 0;
        $scope.ConstructionFilter = "";
      }
    ...
    $scope.Search = function () {
      if ($scope.StartYear < 1975 || $scope.StartYear > d.getFullYear()) {
        toastr.error(
          "Chỉ chấp nhận khoảng thời gian từ năm 1975 đến nay",
          "Lỗi",
        );
        return;
    ...
    function SetInputVal(Id, Data) {
      ...
      if (!input) return;
      input.value = Data ? Data.Name || Data.ConstructionName || "" : "";
    }
    ...
    function popupContent(cons) {
      ...
      return (
        `<div class="card-outline card-outline-tabs map-info-card w-100 border-0">
            <h5 class="card-title text-center col-12 mb-2 font-weight-bold text-deepblue">Thông tin công trình - ` +
        ...
        function () {
          licenseService
            ...
            .then(function (items) {
              ...
              $scope.fromRecord =
                $scope.currentPage > 1
                  ? ($scope.currentPage - 1) * $scope.numPerPage + 1
                  : $scope.currentPage;
            });
        },
        function () {
    ...
    function GetTypeOfConstructionBindToMenu() {
      typeOfConstructionService
        .getTypeOfConstructions(0, Status, "", 0, 0)
        .then(function (items) {
    ...
    function GetTypeOfConstruction(typeid) {
      ...
      if (typeid == 8 || typeid == 9 || typeid == 10) {
        ...
        switch (typeid) {
          case 8:
            item.TypeName = "Khai thác nước dưới đất";
            break;
          case 9:
            item.TypeName = "Thăm dò nước dưới đất";
            break;
          case 10:
            item.TypeName = "Hành nghề khoan nước dưới đất";
            break;
        }
        ...
        typeOfConstructionService
          .getTypeOfConstructions(typeid, Status, "", 0, 0)
          .then(function (items) {
      ...
      licenseService
        ...
        .then(function (items) {
          ...
          if (
            $scope.license &&
            $scope.license.Construction &&
            $scope.license.Construction.CommuneId
          ) {
    ...
    function getCommuneDisplayText(item) {
      ...
      return (item.Name || "") + (item.OldCommuneName || "");
    }

    // When commune changes in the license form, set AdministrativeUnitID on the license object
    $scope.OnCommuneChange = function () {
      if (!$scope.license || !$scope.Communes || !$scope.license.Construction) {
        return;
      }

      var selected = null;
      if ($scope.license.Construction.CommuneId) {
        selected = $scope.Communes.find(function (c) {
          return c.Id === $scope.license.Construction.CommuneId;
        });
      }

      if (!selected && $scope.license.Construction.AdministrativeUnitID) {
        selected = $scope.Communes.find(function (c) {
          return c.Code === $scope.license.Construction.AdministrativeUnitID;
        });
      }

      if (selected) {
        $scope.license.Construction.CommuneId = selected.Id;
        $scope.ConstructionCommuneFilter = getCommuneDisplayText(selected);
        $scope.license.AdministrativeUnitID = selected.Code;
        $scope.license.Construction.AdministrativeUnitID = selected.Code;
      }
    };

    $scope.selectConstructionCommune = function (item) {
      if (!$scope.license.Construction) {
        $scope.license.Construction = {};
      }

      if (!item) {
        $scope.license.Construction.CommuneId = null;
        $scope.license.Construction.AdministrativeUnitID = null;
        $scope.ConstructionCommuneFilter = "";
        return;
      }

      $scope.license.Construction.CommuneId = item.Id;
      $scope.ConstructionCommuneFilter = getCommuneDisplayText(item);
      $scope.OnCommuneChange();
    };

    //data construction
    function GetDataConstruction(typeOfConsId) {
      var selectedTypeOfConsId = typeOfConsId || 0;
      constructionService
        .getAllConstructions(
          selectedTypeOfConsId,
          -1,
          0,
          0,
          0,
    ...
    function countLicense() {
      licenseService.countLicense().then(function (items) {
        ...
        $scope.CountLicenses =
          data.CountLicenses || data.countLicenses || defaultCountLicenses();
      });
    }
    ...
    function drawChart(chartData) {
            ...
            click: function (event, chartContext, config) {
              ...
              switch (config.seriesIndex) {
                case 0:
                  ...
                  break;
                case 1:
                  url += "khai-thac-nuoc-duoi-dat";
                  break;
                case 2:
                  url += "tham-do-nuoc-duoi-dat";
                  break;
                case 3:
                  url += "hanh-nghe-khoan-nuoc-duoi-dat";
                  break;
                case 4:
                  url += "xa-thai";
                  break;
      ...
      setTimeout(function () {
        chart = new ApexCharts(
          document.querySelector("#chartCountNumLicenseFollowYear"),
          options,
        );
        chart.render();
        chart.updateOptions({
    ...
    function init() {
      ...
      switch (pathName) {
        ...
        case "hanh-nghe-khoan-nuoc-duoi-dat":
          $scope.navHeader =
            "DANH SÁCH GIẤY PHÉP HÀNH NGHỀ KHOAN NƯỚC DƯỚI ĐẤT";
      ...
      initMap();
      countLicense();
      // Preload communes so commune selects have data
      GetCommune();
    }
    ...
    $scope.changeLicenseTypeAddNew = function (TypeOfConstructionId) {
      ...
      if (TypeOfConstructionId == 1) $scope.license.ParentId = 0;
    };
    ...
    // Sync selected commune to AdministrativeUnitID on license and construction before save
    function SyncSelectedCommuneToAdminUnit() {
      if ($scope.license && $scope.Communes) {
        var commId =
          $scope.license.Construction && $scope.license.Construction.CommuneId;
        if (commId) {
          var selected = $scope.Communes.find(function (c) {
            return c.Id === commId;
          });
          if (selected) {
            $scope.license.AdministrativeUnitID = selected.Code;
            if (!$scope.license.Construction) $scope.license.Construction = {};
            $scope.license.Construction.AdministrativeUnitID = selected.Code;
          }
        }
      }
    ...
    $scope.addNewCons = function () {
      ...
      $scope.ConstructionCommuneFilter = "";
      // Load commune list when opening the construction modal (no province/district in modal)
      GetCommune();
    };
    ...
    $scope.GetConsData = function (construction) {
      ...
      $scope.license.Construction = angular.copy(construction);
      ...
      constructionService
        ...
        .then(function (consItem) {
          $scope.license.Construction.ConstructionItems =
            consItem.data.ListData;
        });
      inspectionService
        .getAllInspections(construction.Id, true, "", 1, 0)
    ...
    $scope.AddConstructionItem = function () {
      if (
        $scope.license.Construction.ConstructionItems === null ||
        $scope.license.Construction.ConstructionItems == undefined ||
        $scope.license.Construction.ConstructionItems.length == 0
      ) {
        $scope.license.Construction.ConstructionItems = [];
      }
      var item = {
    ...
    $scope.AddMiningPurpose = function () {
      if (
        $scope.license.MiningPurposes === null ||
        $scope.license.MiningPurposes == undefined ||
        $scope.license.MiningPurposes.length == 0
      ) {
    ...
    $scope.RemoveMiningPurpose = function (index, item) {
      ...
      licenseService.DeleteMiningPurpose(item).then(
        function (msg) {
    ...
    $scope.AddInspection = function () {
      if (
        $scope.license.Construction == null ||
        $scope.license.Construction == undefined ||
        $scope.license.Construction.length == 0
      ) {
      ...
      if (
        $scope.license.Construction.Inspection === null ||
        $scope.license.Construction.Inspection == undefined ||
        $scope.license.Construction.Inspection.length == 0
      ) {
    ...
    $scope.RemoveInspection = function (index, item) {
      if (confirm("Xoá biên bản thanh tra này?")) {
        if (item.FilePDF != null) {
          inspectionService
            .DeleteFileInspection(item.FilePDF, GetFolderLicense(typeid))
            .then(function (msg) {
              if (msg) toastr.success("Xóa thành công", "Thanh tra - Kiểm tra");
              else toastr.error("Có lỗi khi xóa", "Thanh tra - Kiểm tra");
            });
        }
        inspectionService.DeleteInspection(item).then(
          function (msg) {
    ...
    $scope.EDIT = function (item, asideId) {
      ...
      if (item.OldLicense !== null) {
        item.OldLicense.SignDate = new Date(
          formatDate(item.OldLicense.SignDate),
        );
      }
      if (item.LicenseFees.length > 0) {
        item.LicenseFees[0].SignDate != undefined
          ? new Date(formatDate(item.LicenseFees[0].SignDate))
          : null;
      ...
      if (confirm("Xoá giấy phép " + item.LicenseNumber + " ?")) {
        if (item.LicenseFile != null) {
          licenseService
            .DeleteLicenseFile(
              item.LicenseFile,
              item.RelatedDocumentFile,
              GetFolderLicense(TypeOfConstructionId),
            )
            .then(function (msg) {
              if (msg) {
        ...
        licenseService.DeleteLicense(item).then(
          function (msg) {
            GetDataLicenses();
            toastr.success("Xóa thành công", "Giấy phép");
          },
    ...
    function CheckRequired() {
      if (
        $scope.license.LicenseNumber === null ||
        $scope.license.LicenseNumber === undefined ||
        $scope.license.LicenseNumber === ""
      )
        return "Số giấy phép";
      if (
        $scope.license.LicensingTypeId === null ||
        $scope.license.LicensingTypeId === undefined ||
        $scope.license.LicensingTypeId === ""
      )
        return "Loại giấp phép";
      if (
        $scope.license.SignDate === null ||
        $scope.license.SignDate === undefined ||
        $scope.license.SignDate === ""
      )
        return "Ngày ký giấy phép";
      if (
        $scope.license.IssueDate === null ||
        $scope.license.IssueDate === undefined ||
        $scope.license.IssueDate === ""
      )
        return "Ngày giấy phép có hiệu lực";
      if (
        $scope.license.Duration === null ||
        $scope.license.Duration === undefined ||
        $scope.license.Duration === ""
      )
      ...
      if (
        $scope.license.ExpireDate === null ||
        $scope.license.ExpireDate === undefined ||
        $scope.license.ExpireDate === ""
      )
        return "Ngày giấy phép hết hạn";
      if (
        $scope.license.Business.Name === null ||
        $scope.license.Business.Name === undefined ||
        $scope.license.Business.Name === ""
      )
        return "Tên chủ giấy phép";
      if (
        $scope.license.Business.Address === null ||
        $scope.license.Business.Address === undefined ||
        $scope.license.Business.Address === ""
      )
        return "Địa chỉ chủ giấy phép";
      if (
        $scope.license.Construction.ConstructionName === null ||
        $scope.license.Construction.ConstructionName === undefined ||
        $scope.license.Construction.ConstructionName === ""
      )
        return "Công trình cấp phép";
      return false;
    }

    function SetDateWithNoon(date) {
      if (!date) return null;
      let d = new Date(date);
    ...
    $scope.Save = function (formIdClose) {
      // Ensure selected commune is synced to AdministrativeUnitID before save
      SyncSelectedCommuneToAdminUnit();
      let GetFileAttachsLicense =
        document.getElementById("fileAttachsLicense").files;
      ...
      if (
        ...
        $scope.license.SignDate = SetDateWithNoon($scope.license.SignDate);

      if (
        $scope.license.ExpireDate !== undefined &&
        $scope.license.ExpireDate !== null
      )
        $scope.license.ExpireDate = SetDateWithNoon($scope.license.ExpireDate);

      if (
        $scope.license.IssueDate !== undefined &&
        $scope.license.IssueDate !== null
      )
        $scope.license.IssueDate = SetDateWithNoon($scope.license.IssueDate);

      //save cons
      var newCons = $scope.license.Construction;

      newCons.TypeOfConstructionId =
        TypeOfConstructionId == 8
          ? 8
          : TypeOfConstructionId == 9
            ? 9
            : TypeOfConstructionId == 10
              ? 10
              : $scope.license.Construction.TypeOfConstructionId;
      constructionService.SaveConstruction(newCons).then(
        function (msg) {
          //save license
          if (msg.data.Id > 0) {
            ...
            if (
              $scope.license.Business !== null &&
              $scope.license.Business.length !== 0 &&
              $scope.license.Business !== undefined
            ) {
              SaveBusiness();
            }
            ...
            if ($scope.license.Construction.ConstructionItems !== undefined) {
              if ($scope.license.Construction.ConstructionItems !== null) {
                $scope.license.Construction.ConstructionItems.forEach(
                  function (conItem) {
                    conItem.ConstructionId = msg.data.Id;
                    conItem.AquiferId = msg.data.AquiferId;
                    constructionService.SaveConstructionDetail(conItem).then(
                      function (msg) {
            ...
            if ($scope.license.Construction.Inspection != null) {
              ...
              $scope.license.Construction.Inspection.forEach(
                function (item, index) {
                  if (
    ...
    function SaveBusiness() {
      ...
      businessService.SaveBusiness(newBusiness).then(
        function (msg) {
          //save license
          if (msg.data.Id > 0) {
    ...
    function SaveLicense(TypeOfConstructionId, BusinessId) {
      let GetFileAttachsLicense =
        document.getElementById("fileAttachsLicense").files;
      let GetFileAttachsRelated =
        document.getElementById("fileAttachsRelated").files;
      let GetFileAttachsLicenseRequest = document.getElementById(
        "fileAttachsLicenseRequest",
      ).files;
      ...
      if (GetFileAttachsLicense !== undefined) {
        // veryfile type pdf or png or ..
        ...
        if (GetFileAttachsLicense.length > 0) {
          for (let i = 0; i < GetFileAttachsLicense.length; i++) {
            ...
            if (str.includes("pdf", str.length - 4)) {
              ...
              toastr.warning(
                "File " + str + " không được chấp nhận. Vui lòng chọn File PDF",
                "Giấy phép",
              );
              return;
            }
      ...
      if (GetFileAttachsRelated !== undefined) {
        // veryfile type pdf or png or ..
        // Add the uploaded file to the form data collection
      ...
      if (GetFileAttachsLicenseRequest !== undefined) {
        // veryfile type pdf or png or ..
        // Add the uploaded file to the form data collection
        if (GetFileAttachsLicenseRequest.length > 0) {
          for (let i = 0; i < GetFileAttachsLicenseRequest.length; i++) {
            ...
            if (str.includes("pdf", str.length - 4)) {
              data.append(
                "LicenseRequestFile",
                GetFileAttachsLicenseRequest[i],
              );
            } else {
      ...
      if (FileAttachsLicense !== undefined) {
        // Save file on sql frist then to local if file not undefined
        if (FileAttachsLicense.length > 0) {
          if ($scope.Action === "Update") {
            licenseService.DeleteLicenseFile(
              $scope.license.LicenseFile,
              $scope.license.RelatedDocumentFile,
              GetFolderLicense(TypeOfConstructionId),
            ); // xoa file cu khi update file moi
          }
          licenseService.SaveLicense($scope.license).then(
            function (msg) {
              ...
              if (
                $scope.license.LicenseFee !== null &&
                $scope.license.LicenseFee.length !== 0 &&
                $scope.license.LicenseFee !== undefined
              ) {
                if (
                  $scope.license.LicenseFee.SignDate !== undefined &&
                  $scope.license.LicenseFee.SignDate !== null
                ) {
                  $scope.license.LicenseFee.SignDate = new Date(
                    $scope.license.LicenseFee.SignDate,
                  );
                }

                if ($scope.license.LicenseFee !== undefined) {
                  $scope.license.LicenseFee.LicensingAuthorities =
                    msg.data.LicensingAuthorities;
                }
          ...
          if ($scope.Action === "Update") {
            licenseService.SaveLicense($scope.license).then(
              function (msg) {
                ...
                document.getElementById("fileAttachsLicense").value = null;
                document.getElementById("fileAttachsRelated").value = null;
                document.getElementById("fileAttachsLicenseRequest").value =
                  null;
              },
              function () {
    ...
    function SaveLinceseFee(LicenseId) {
      ...
      if (myfileAttach !== undefined) {
        ...
        if (myfileAttach.length > 0) {
          ...
          licenseFeeService.SaveLicenseFee(licFee, LicenseId).then(
            function (msg) {
              if (licFee != null) {
                let licLicF = {
                  LicenseId: LicenseId,
                ...
                licenseService.SaveLicenseLicenseFee(licLicF).then(
                  function (msg) {
                    toastr.success(
                      "Lưu thành công",
                      "Tiền cấp quyền - Giấy phép",
                    );
                  },
                  function (err) {
              ...
              $.ajax({
                type: "POST",
                url:
                  "/api/LicenseFee/upload?Id=" + msg.data.Id + "&Name=" + Name,
                contentType: false,
                processData: false,
                dataType: "JSON",
                data: data,
      ...
      if ($scope.license.MiningPurposes !== undefined) {
        if ($scope.license.MiningPurposes !== null) {
          $scope.license.MiningPurposes.forEach(function (e) {
            ...
            licenseService.SaveMiningPurpose(e).then(
              function (msg) {
    ...
    function SaveInspection(TypeOfConstructionId, index, item) {
        ...
        if (files.length > 0) {
          for (let i = 0; i < files.length; i++) {
            ...
            if (str.includes("pdf", str.length - 4)) {
              ...
              document.getElementById("fileAttachsInspection" + index).value =
                null;
              return;
            }
          }
      ...
      if (myfileAttach !== undefined) {
        ...
        if (myfileAttach.length > 0) {
          if ($scope.Action === "Update") {
            inspectionService.DeleteFileInspection(
              item.FilePDF,
              GetFolderLicense(TypeOfConstructionId),
            ); // xoa file cu khi update file moi
          }
          inspectionService.SaveInspection(item).then(
            function (msg) {
              let Id = msg.data.Id;
              $.ajax({
                type: "POST",
                url:
                  "/api/Inspection/upload?Id=" +
                  Id +
                  "&FolderName=" +
                  GetFolderLicense(TypeOfConstructionId),
                contentType: false,
    ...
    $scope.AddIconToAnnotate = function (slug) {
      if (slug == "" || slug == null) {
        ...
        return (
          '<img width=25 src="http://waterresources.loc/LocalFiles/images/ICON_GHICHUCONGTRINH/' +
          slug +
          '.png" />'
        );
      }
    ...
    // Get file pdf
    $scope.openAsideFileLicense = function (TypeOfConsId, FilePDF, LicenseNumber, SignDate) {
      openAsideFile();
      $scope.fileSource =
    ...
    function getLicenseDownloadFileName(fileName, licenseNumber, signDate) {
      if (!licenseNumber || !signDate) return fileName;
      ...
      var safeLicenseNumber = licenseNumber
        .toString()
        .replace(/[\\/:*?"<>|]/g, "-")
        .replace(/\s+/g, " ")
        .trim();

      return safeLicenseNumber + "_" + dd + "-" + mm + "-" + yyyy + ".pdf";
    }
